<a href="{{ route('prestasi.edit', $pre->id) }}"><button class="btn btn-sm btn-primary"> <i class="fa fa-pencil"></i></button></a>
<a href="javascript:void(0);" onclick="if(confirm('Anda yakin ingin menghapus?')) {$(this).find('form').submit()};" >
    ...
    <form class="" action="{{ route('prestasi.destroy', $pre->id) }}" method="post">
        {{ csrf_field() }}
        {{ method_field('DELETE')}}    
    </form>
</a>
